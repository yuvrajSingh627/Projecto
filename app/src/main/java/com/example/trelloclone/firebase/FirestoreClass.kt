...
import com.example.trelloclone.activities.SignInAcitivity
...
class FirestoreClass {
    ...
    fun registerUser(activity: SignUpActivity, userInfo: User){
        mFireStore.collection(Constants.USERS)
            .document(getCurrentUserID()).set(userInfo, SetOptions.merge()).addOnSuccessListener { activity.userRegisteredSuccess() }
            ...
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    ...
                    if (loggedInUser != null) {
                        when (activity) {
                            is SignInAcitivity -> activity.signInSuccess(loggedInUser)
                            ...
                            else -> Log.e("FirestoreClass", "Unexpected activity type: ${activity::class.java.simpleName}")
                        }
                    } else {
                        Log.e("FirestoreClass", "LoggedInUser is null")
                    }
                } else {
                    Log.e("FirestoreClass", "Document does not exist")
                }
            }
            .addOnFailureListener { e ->
                when (activity) {
                    is SignInAcitivity -> activity.hideProgressDialog()
